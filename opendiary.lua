...
allowed = function(url, parenturl)
  ...
  if not string.match(url, "^https?://[^/]*opendiary%.com/") then
...
wget.callbacks.get_urls = function(file, url, is_css, iri)
  ...
  local function check(newurl)
    ...
    if string.match(url_, "%s") then
      for x in string.gmatch(url_, "([^%s]+)") do
        check(x)
      end
      return nil
    end
    if string.match(url_, "^http://[^/]*opendiary%.com/") then
      return check("https://" .. string.match(url_, "^http://(.+)$"))
    end
  ...
  if allowed(url)
    ...
    html = read_file(file)
    if item_type == "author" then
      check("https://www.opendiary.com/members/" .. item_value .. "/")
    end
...
wget.callbacks.finish = function(start_time, end_time, wall_time, numurls, total_downloaded_bytes, total_download_time)
  ...
  for key, data in pairs({
    ["opendiary-g60s27ky1jm1z6ww"] = discovered_items,
    ["urls-bhmkj5cphzo1aj7a"] = discovered_outlinks
  }) do
